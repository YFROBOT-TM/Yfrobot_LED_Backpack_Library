/*!
 * @file Yfrobot_LEDBackpack.cpp
 *
 * @mainpage Yfrobot LED Backpack library for Arduino.
 * 
 * @Revised by YFROBOT
 * 
...
void Yfrobot_LEDBackpack::setDisplayState(bool state) {
  uint8_t buffer;
  if (state)
...
void Yfrobot_LEDBackpack::setBrightness(uint8_t b) {
  if (b > 15)
...
void Yfrobot_LEDBackpack::blinkRate(uint8_t b) {
  if (b > 3)
...
Yfrobot_LEDBackpack::Yfrobot_LEDBackpack(void) {}

bool Yfrobot_LEDBackpack::begin(uint8_t _addr, TwoWire *theWire) {
  if (i2c_dev)
    delete i2c_dev;
...
void Yfrobot_LEDBackpack::writeDisplay(void) {
  uint8_t buffer[17];
...
void Yfrobot_LEDBackpack::clear(void) {
  for (uint8_t i = 0; i < 8; i++) {
    displaybuffer[i] = 0;
  }
}
...
/******************************* 10 BARGRAPH object of YFROBOT 20240624 */
Yfrobot_10bargraph::Yfrobot_10bargraph(void) {}

void Yfrobot_10bargraph::setBar(uint8_t bar) {
  uint16_t a;
  a = bar;
  if (a > 9)  a = 9; // limit to max
  Serial.print("Ano = "); Serial.println(a);
  // Turn on LED.
  displaybuffer[0] |= _BV(a);
}

void Yfrobot_10bargraph::clearBar(uint8_t bar) {
  uint16_t a;
  a = bar;
  if (a > 9)  a = 9; // limit to max
  Serial.print("Ano2 = "); Serial.println(a);
  // Turn off LED.
  displaybuffer[0] &= ~_BV(a);
}
...
/******************************* 3-bit 7-segment object of YFROBOT 20240613 */

Yfrobot_3bit_7segment::Yfrobot_3bit_7segment(void) { position = 0; }

void Yfrobot_3bit_7segment::print(const String &c) { write(c.c_str(), c.length()); }

void Yfrobot_3bit_7segment::print(const char c[]) { write(c, strlen(c)); }

void Yfrobot_3bit_7segment::print(char c) { write(c); }

void Yfrobot_3bit_7segment::print(unsigned long n, int base) {
  if (base == 0)
    write(n);
  else
    printNumber(n, base);
...
void Yfrobot_3bit_7segment::print(unsigned char b, int base) {
  print((unsigned long)b, base);
}

void Yfrobot_3bit_7segment::print(int n, int base) { print((long)n, base); }

void Yfrobot_3bit_7segment::print(unsigned int n, int base) {
  print((unsigned long)n, base);
}

void Yfrobot_3bit_7segment::print(double n, int digits) { printFloat(n, digits); }

size_t Yfrobot_3bit_7segment::write(char c) {
...
size_t Yfrobot_3bit_7segment::write(const char *buffer, size_t size) {
  size_t n = 0;
...
void Yfrobot_3bit_7segment::writeDigitRaw(uint8_t d, uint8_t bitmask) {
  if (d > 4)
    return;
  displaybuffer[d] = bitmask;
}

void Yfrobot_3bit_7segment::drawColon(bool state) {
  if (state)
    displaybuffer[2] = 0x2;
  else
    displaybuffer[2] = 0;
...
void Yfrobot_3bit_7segment::writeColon(void) {
  uint8_t buffer[3];
...
void Yfrobot_3bit_7segment::writeDigitNum(uint8_t d, uint8_t num, bool dot) {
  if (d > 4 || num > 15)
    return;
...
void Yfrobot_3bit_7segment::writeDigitAscii(uint8_t d, uint8_t c, bool dot) {
  if (d > 4)
    return;
...
void Yfrobot_3bit_7segment::print(long n, int base) { printNumber(n, base); }

void Yfrobot_3bit_7segment::printNumber(long n, uint8_t base) {
  printFloat(n, 0, base);
}

void Yfrobot_3bit_7segment::printFloat(double n, uint8_t fracDigits, uint8_t base) {
...
void Yfrobot_3bit_7segment::printError(void) {
  for (uint8_t i = 0; i < SEVENSEG_DIGITS; ++i) {
    writeDigitRaw(i, (i == 2 ? 0x00 : 0x40));
  }
}
